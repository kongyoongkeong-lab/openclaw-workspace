...
- If Jason says `指令体检`, `快捷体检`, `Shortcut体检`, or `shortcut audit`, run `/home/jason2ykk/.openclaw/workspace/tools/shortcut_doctor.py audit --deep` in read-only mode and summarize missing local paths, warning counts, safety-class counts, help/self-test/dry-run hints, and report paths. Do not execute shortcut target commands during this audit.
- If Jason says `快捷状态`, `/shortcut_status`, or `shortcut status`, run `/home/jason2ykk/.openclaw/workspace/tools/shortcut_status.py status` in read-only mode. If a group is included (`photo`, `model`, `backup`, `automation`, `system`, `news`, `github`), pass it as the optional group argument. Summarize total, by_status, by_risk, needs_confirm, quarantined, and recent failures. Do not execute shortcut target commands.
- Before asking Jason to approve a follow-up with a short confirmation, run `/home/jason2ykk/.openclaw/workspace/tools/pending_action.py ask --id <task-id> --title "<short title>" --summary "<safe no-secret summary>" --risk <low|medium|high> --source telegram-prompt` so `confirm proceed` has a durable local target.
- If Jason says `confirm proceed`, `conifrm proceed`, `确认继续`, or another short confirmation reply, first run `/home/jason2ykk/.openclaw/workspace/tools/pending_action.py consume --source telegram-confirm`. If it returns no pending action or an expired action, do not infer approval unless the immediately preceding visible conversation explicitly identifies the target task. Never store secrets in `config/pending_action.json`.
...
- If Jason says `重启网关`, first run `/home/jason2ykk/.openclaw/workspace/tools/pending_action.py ask --id gateway-restart --title "Restart OpenClaw Gateway" --summary "Restart OpenClaw gateway; active sessions may disconnect briefly." --risk medium --source telegram-prompt` and ask Jason to reply `confirm proceed`. Only after confirmation should `openclaw gateway restart` run.
