"""
alerting_system_final.py - Runtime Safety Enforcement Layer
...
class AlertingSystem:
    ...
    def check_gpu_pressure(self, gpu_usage_percent: int, device: Optional[str] = None) -> Optional[GPUAlert]:
        ...
        if gpu_usage_percent >= self.gpu_thresholds["emergency"]:
            ...
            self.record_alert(GPUAlert, **asdict(alert))
    ...
    def check_event_loop(self, event_loop_delay_ms: int, inference_queue_age: int = 0,
        ...
        if event_loop_delay_ms > 1000:
            ...
            self.record_alert(EventLoopAlert, **asdict(alert))
            if event_loop_delay_ms > 3000:
                self.recovery_actions["pause_noncritical_agents"]()
                alert.action_taken = "pause_noncritical_agents"
                self.record_alert(EventLoopAlert, **asdict(alert))
            return alert
    ...
    def classify_agent_failure(self, failure_type: str, severity: str, agent_id: Optional[str] = None,
        ...
        self.record_alert(AgentFailureAlert, **asdict(alert))
...
if __name__ == "__main__":
    ...
    # Test GPU alerts
    alert = system.check_gpu_pressure(gpu_usage)
    if alert:
        print(f"Alert: {alert.level} - Usage: {alert.usage_percent}%")
    else:
        print("No alerts triggered (usage within thresholds)")

    # Test other alerts
    print("\nTesting event loop stall...")
    system.check_event_loop(event_loop_delay_ms=1500)

    print("\nTesting compression event...")
    system.log_compression_event("retention_policy_update", compression_ratio=0.6)

    print("\nTesting agent failure...")
    system.classify_agent_failure("timeout", severity="medium", agent_id="@intel")

    print("\nAlert statistics:")
    print(system.get_alert_stats())